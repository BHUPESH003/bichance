...
const AuthPage = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      ...
      } else if (res.ok && (data.success || data.message?.toLowerCase().includes('otp'))) {
        setStep(2);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-red-50 to-white px-2 sm:px-4 w-full">
      <motion.div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 w-full max-w-md" initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }}>
        <h2 className="text-3xl font-bold text-center text-red-600 mb-6">Sign Up with Email</h2>
        {step === 1 && (
          <form onSubmit={handleSendOtp} className="flex flex-col gap-4">
            ...
            >
              {loading ? 'Sending OTP...' : countdown > 0 ? `Wait ${countdown}s` : 'Send OTP'}
            </button>
        ...
        {step === 2 && (
          <form onSubmit={handleVerifyOtp} className="flex flex-col gap-4">
            ...
            <input
              ...
              required
            />
            <button type="submit" className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-lg transition-all text-lg mt-2" disabled={loading}>
              {loading ? 'Verifying...' : 'Verify OTP'}
            </button>
            {error && <div className="text-red-600 text-center font-semibold">{error}</div>}
            {success && <div className="text-green-600 text-center font-semibold">{success}</div>}
            <button type="button" className="text-red-500 underline mt-2" onClick={() => setStep(1)}>
              Change Email
            </button>
          </form>
        )}
